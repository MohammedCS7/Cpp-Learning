...
public:
	...
	{
		return GetSystemDate().ToString() + " - " + GetSystemTime();
	}
...
public:
	...
	static void PrintYearCalendar()
	{
		for (int i = 1; i <= 12; i++)
		{
			PrintMonthCalendar(Date(1, i, 1));
		}
	}

	static bool IsBefore(const Date& date1, const Date& date2)
	{
	...
	static bool IsEqual(const Date& date1, const Date& date2)
	{
		return (date1._Year == date2._Year && date1._Month == date2._Month && date1._Day == date2._Day);
	}

	static bool IsAfter(const Date& date1, const Date& date2)
	{
		return !(IsBefore(date1, date2) || IsEqual(date1, date2));
	}

	bool IsBefore(const Date& date2) const
	{
	...
	bool IsAfter(const Date& date2) const
	{
	...
	static string GetSystemTime()
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		string timeStr = to_string(now->tm_hour) + ":" + to_string(now->tm_min) + ":" + to_string(now->tm_sec);
		return timeStr;
	}
	...
	static Date GetMin(const Date& date1, const Date& date2)
	{
		return date1.IsBefore(date2) ? date1 : date2;
	}

	Date GetMin(const Date& date2)
	{
		return GetMin(*this, date2);
	}

	static Date GetMax(const Date& date1, const Date& date2)
	{
		return date1.IsAfter(date2) ? date1 : date2;
	}

	Date GetMax(const Date& date2)
	{
		return GetMax(*this, date2);
	}
	...
	static int GetDiffBetweenDate(const Date& date1, const Date& date2,bool IncludeCurrentDay = true)
	{
		if (IsAfter(date1, date2))
		{
			return -GetDiffBetweenDates(date2, date1, IncludeCurrentDay);
		}
		else if (IsBefore(date1, date2))
		{
			return GetDiffBetweenDates(date1, date2, IncludeCurrentDay);
		}
		else
			return IncludeCurrentDay ? 1 : 0;
	}
	...
	{
		return IsAfter(date1, date2) ? DateState::After : IsBefore(date1, date2) ? DateState::Before : DateState::Equal;
	}
