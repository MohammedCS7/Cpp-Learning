...
Date IncreaseDateForVacDays(Date date, int days)
{
    for (int i = 0; i < days;)
    {
        if (IsBusinnessDay(GetDayOrder(date)))
            i++;
...
int main()
{
    ...
    int days = ReadPositiveNumber("Please enter vacation days : ");

    Date vacEnd = IncreaseDateForVacDays(vacStart, days);
